...
class ParallelUNet(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.pose_to_cond = nn.ModuleList([nn.Linear(2, context_dim) for _ in range(1)])
        ...
        self.null_pose_hidden = nn.ParameterList([nn.Parameter(torch.randn(1, time_cond_dim)) for _ in range(1)])
    ...
    def forward(
        ...
        *,
        ...
        poses_keep_mask = [prob_mask_like((batch_size,), 1 - cond_drop_prob, device=device) for _ in range(1)]
        ...
        c = []
        for i, pose in enumerate([person_poses]):
...
class TryOnImagenlite(nn.Module):
    def __init__(
        self,
        unets,
        *,
...
def get_unet_by_name_lite(name: str, **model_kwargs):
    if name == "base":
